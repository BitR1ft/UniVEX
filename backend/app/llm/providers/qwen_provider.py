...
OpenAI-compatible API for Qwen models via DashScope.
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        if response.status_code == 401:
            raise PermissionError(f"Qwen API authentication failed: {response.text}")
        if response.status_code == 429:
            raise RuntimeError(f"Qwen API rate limit exceeded: {response.text}")
